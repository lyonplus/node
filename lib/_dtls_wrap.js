...
var tls = require('tls');
var _tls_common = require('_tls_common');
...
DTLSSocket.prototype._init = function () {    
  ...
  var credentials = options.credentials || tls.createSecureContext();
...
Server.prototype.addContext = function(servername, credentials) {
  ...
  var re = new RegExp('^' +
                      servername.replace(/([\.^$+?\-\\[\]{}])/g, '\\$1')
                                .replace(/\*/g, '.*') +
                      '$');
  this._contexts.push([re, tls.createSecureContext(credentials).context]);
};
